//: [Episode 02: Enumerations](@previous)
...
enum Suit: String {
    case heart = "suit.heart.fill", club = "suit.club.fil", spade = "suit.spade.fill", diamond = "suite.diamond.fill"
}
...
let coinPurse: [Coin] = [.penny, .nickel, .dime, .quarter, .quarter, .nickel, .penny, .quarter]
...
let total = coinPurse.reduce(0) { (result, coin) in
    result + coin.rawValue
}
print(total)
//: [Episode 04: Switch Statements](@next)
